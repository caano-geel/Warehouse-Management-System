...
if ($barang->stock <= $limitstock->stock) {
	...
	public function laporanmasukpdf(){
		...
		$data['admin']					= $this->ADM->get_admin('',$where_admin);
   		...
   		$this->dompdf_gen->stream("incomingoods.pdf", array('Attachment'=>1));
	}
	...
	public function laporankeluarpdf(){
		...
		$data['admin']					= $this->ADM->get_admin('',$where_admin);
   		...
   		$this->dompdf_gen->load_html($html);
   		$this->dompdf_gen->render();
   		$this->dompdf_gen->stream("outgoods.pdf", array('Attachment'=>1));
	}
	...
	public function laporanpenyesuaianpdf(){
		...
		$data['admin']					= $this->ADM->get_admin('',$where_admin);
   		...
   		$this->dompdf_gen->load_html($html);
   		$this->dompdf_gen->render();
   		$this->dompdf_gen->stream("in&outgoods.pdf", array('Attachment'=>1));
	}
