...
class Excel_generator extends Spreadsheet {
    public function __construct() {
        if ( ! class_exists(Spreadsheet::class)) {
            show_error('PhpSpreadsheet is not installed. Run "composer install" in the project root before exporting spreadsheets.');
        }
    ...
    public function exportTo2003($filename = 'doc') {
        $this->writeToFile($filename . '.xls', 'Xls');
    }
    /**
     * @param string $filename
     */
    public function exportTo2007($filename = 'doc') {
        $this->writeToFile($filename . '.xlsx', 'Xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    }
}
